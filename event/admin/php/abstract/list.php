	...
	<h2>Abstract 관리</h2>
		<div class="btnArea" style="width:100%;">
			...
			<?if($event_db['session_sync']=='Y') {?>
				<a class="btnOrg tooltipPoint" title="초록 미리보기" href="" onclick="javascript:view_sync('<?=$code?>')"  ><i class="fas fa-eye"></i>미리보기</a>
			<?}else{?>
		...
		<div class="contents member">
			<table class="tblList">
				...
				<tr class="bg" id="<?=$col['sid']?>">
					...
					<td><input onchange="changeVal(this,'speaker','<?=$col['sid']?>')" type="text" name="speaker" id="speaker"  value="<?=$col['speaker']?>" /></td>
					<td><input onchange="changeVal(this,'speaker_office','<?=$col['sid']?>')" type="text" name="speaker_office" id="speaker_office"  value="<?=$col['speaker_office']?>" /></td>
					...
					<?}?>
					<?if($as['abs_info4']){?>
						<td><input onchange="changeVal(this,'info4','<?=$col['sid']?>')" type="text" name="info4" id="info4"  value="<?=htmlspecialchars($col['info4'])?>" /></td>
					<?}?>
					<?if($as['abs_info5']){?>
						<td><input onchange="changeVal(this,'info5','<?=$col['sid']?>')" type="text" name="info5" id="info5"  value="<?=htmlspecialchars($col['info5'])?>" /></td>
					<?}?>
					<?if($as['abs_info6']){?>
					...
					<?}?>
					<?if($as['abs_info9']){?>
						<td><input onchange="changeVal(this,'info9','<?=$col['sid']?>')" type="text" name="info9" id="info9"  value="<?=htmlspecialchars($col['info9'])?>" /></td>
					<?}?>
					<?if($as['abs_info10']){?>
...
function view_sync(code) {
	window.open("/voting_sync/php/abstract/category.php?code="+code,"","width=530,height=800");
}
