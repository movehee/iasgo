...
<div id="container">
		...
		<div class="contents member">
			...
			<li><a href="<?=$_SERVER['PHP_SELF']?>?tab=<?=$tab?>&page=0<?if($search){?>&search=<?=$search?><?}?>"><i class="fas fa-angle-double-left"></i></a></li>
			<li><a href="<?=$_SERVER['PHP_SELF']?>?tab=<?=$tab?>&page=<?if($page-$num_per_page2>0){ echo $page-3;}else{echo "0";}?><?if($search){?>&search=<?=$search?><?}?>"><i class="fas fa-angle-left"></i></a></li>
			<?

				for($i = $s; $i < $e ; $i++)
				{?>	
					<li <?if($i==$page){?>class="on"<?}?>><a href="<?=$_SERVER['PHP_SELF']?>?tab=<?=$tab?>&page=<?=$i?><?if($search){?>&search=<?=$search?><?}?>"><?=$i+1?></a></li>
				<?}
			?>

			<li><a href="<?=$_SERVER['PHP_SELF']?>?tab=<?=$tab?>&page=<?if($page+$num_per_page2>$max-1){echo $max-1;}else{echo $page+$num_per_page2;}?><?if($search){?>&search=<?=$search?><?}?>"><i class="fas fa-angle-right"></i></a></li>
			<li><a href="<?=$_SERVER['PHP_SELF']?>?tab=<?=$tab?>&page=<?=$max-1?><?if($search){?>&search=<?=$search?><?}?>"><i class="fas fa-angle-double-right"></i></a></li>
			</ul>
...
function event_code(code) {
	location.href="./list.php?code="+code;
}
