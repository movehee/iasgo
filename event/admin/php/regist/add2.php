...
<form enctype="multipart/form-data" name="registform" id="registform" action="./post.php" method="post"> 
	<table class="registList">	
		...
		if($regist_d['parent'] && !$isadmin){?>
			<input type="hidden" name="parent_<?=$regist_d['parent']?>" id="parent_<?=$regist_d['parent']?>" value="<?=$regist_d['parent_val']?>" />	
		<?
		...
		?>
		<tr class="parent_<?=$regist_d['parent']?>" id="<?if($regist_d['countrychk']=="1"){?>kor<?}else if($regist_d['countrychk']=="2"){?>eng<?}?>" style="<?=$style_txt?>" <?if($regist_d['parent']){?>class="parent<?=$regist_d['parent']?> <?=$regist_d['parent_val']?>"<?}?>>
			...
			<td class="regist_td <?if($regist_d['type']=="30"){?>small<?}?>">
				...
				<?
					...
					<?if($val_type=="1"){?>
						...
						<input <?if($onchangechk){?>onchange="money_change('<?=$regist_d['info_orderby']?>')"<?}?> <?if($d['info'.$regist_d['info_orderby']]==$type_d['sid']){?>checked<?}?>  type='radio' name='val_<?=$regist_d['info_orderby']?>' value="<?=$type_d['sid']?>"/><?=$type_d['info']?>
					<?}else if($val_type=="2"){
						$temp = split(",",$d['info'.$regist_d['info_orderby']]);

					?>
						
						<input <?if($onchangechk){?>onchange="money_change('<?=$regist_d['info_orderby']?>')"<?}?> type='checkbox' name='val_<?=$regist_d['info_orderby']?>[]' value="<?=$type_d['sid']?>"  <?
						...
						?>/><?=$type_d['info']?>
					<?}else if($val_type=="3"){?>
						<option <?if($d['info'.$regist_d['info_orderby']]==$type_d['sid']){?>selected<?}?> value="<?=$type_d['sid']?>"><?=$type_d['info']?></option>
					<?}
					$k++;
					}?>
					<?if($val_type=="3"){?>
