...
if($indate){
	...
	$a_result = mysqli_query($conn, "select * from agenda_tbl where sid='".$indate."'");
		...
		<div class="contents member">
			...
			<table class="inputTbl">
				...
				<tr>
					...
					<td><select onchange="javascript:search_select('<?=$code?>')" name="eventdate" id="eventdate">
						...
						while(is_array($tab_col = mysqli_fetch_array($tab_result))){?>
							<option<?=$eventdate==$tab_col['eventdate']?' selected="true"':''?> value="<?=$tab_col['eventdate']?>"><?=$tab_col['name']?></option>
						<?}mysqli_data_seek($tab_result,0);?>
				...
				<tr>
					...
					<td><select onchange="javascript:search_select('<?=$code?>')" name="indate" id="indate">
						...
						while(is_array($tab_col = mysqli_fetch_array($tab_result))){?>
							<option<?=$indate==$tab_col['sid']?' selected="true"':''?> value="<?=$tab_col['sid']?>"><?=$tab_col['name']?></option>
						<?}mysqli_data_seek($tab_result,0);?>
					...
					<td><p class="btn"><a href="./list.php?code=<?=$code?>" class="btnDef">초기화</a></p></td>
...
function search_select(code){
	//alert(code);
	//alert("./list.php?code="+code+"&reg_money_gubun="+document.getElementById("reg_money_gubun").value+"&pay_chk="+document.getElementById("pay_chk").value+"&eventdate="+document.getElementById("eventdate").value);

	location.replace("./list.php?code="+code+"&reg_money_gubun="+document.getElementById("reg_money_gubun").value+"&pay_chk="+document.getElementById("pay_chk").value+"&eventdate="+document.getElementById("eventdate").value+"&indate="+document.getElementById("indate").value+"&keyword="+document.getElementById("keyword").value+"&memoYN="+document.getElementById("memoYN").value);
}
