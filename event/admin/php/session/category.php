...
$query="SELECT * FROM session_category_tbl where code='".$code."' and del='N'";

$result = mysqli_query($conn, "SELECT count(*) cnt FROM session_category_tbl where code='".$code."'");
$row = mysqli_fetch_array($result);
$totalRecord = $row['cnt'];

$query.=" order by orderby asc LIMIT ".$page*$num_per_page.",".$num_per_page;

$result = mysqli_query($conn, "select max(orderby) maxs from session_category_tbl where code='".$code."'");
$row = mysqli_fetch_array($result);
$max = $row['maxs'];
		...
		<div class="contents member">
			...
			<table class="tblList">
				...
				<tr class="bg" id="<?=$col['sid']?>">
					...
					<td><?=$col['info']?></td>
					<td><?=$col['color']?></td>
					<td><?if($col['glance_type']=="1"){echo "Icon";}else if($col['glance_type']=="2"){echo "BG";}?></td>
					<td><?=$col['glance_color']?></td>
					<td>
