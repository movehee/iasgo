...
<div id="container" style="width:1700px">
		...
		<div class="btnArea" style="width:100%;">
			<span class="btn" style="float:right; margin-right:10px;">
				...
				<?}else{?>
					...
					<?if($s['glance_type']=='M'){?>
					<a href=""title="glance 미리보기"  onclick="javascript:view_maker('<?=$code?>','<?=$tab?>')" class="btnOrang"><i class="fas fa-eye"></i>Glance Maker</a>
					<?}?>
		...
		<div class="contents member">
			...
			<span style="float:left;">
				<select name="room" style=" font-size:13px; width: 140px;  height: 40px;"  onchange="location.href='list2.php?code=<?=$code?>&tab=<?=$tab?>&category1=<?=$category1?>&time=<?=$time?>&room='+this.value">
			...
			<span style="float:left;padding-left:5px;">
				<select name="category1" style=" font-size:13px; width: 140px;  height: 40px;"  onchange="location.href='list2.php?code=<?=$code?>&tab=<?=$tab?>&room=<?=$room?>&category1=<?=$category1?>&time='+this.value">
					<option value="">Time</option>
					<?
					while(is_array($time_d = mysqli_fetch_array($time_result))){?>
						<option<?=$time==$time_d['sid']?' selected="true"':''?> value="<?=$time_d['sid']?>"><?=$time_d['time']?></option>
					<?}
			...
			<?if($s['category1']=="Y"){?>
			<span style="float:left;padding-left:5px;">
				<select name="category1" style=" font-size:13px; width: 140px;  height: 40px;"  onchange="location.href='list2.php?code=<?=$code?>&tab=<?=$tab?>&room=<?=$room?>&time=<?=$time?>&category1='+this.value">
			...
			<table class="tblList bgOver">
				...
				<thead>
					<tr>
						...
						<?}?>
						<?if($s['sub_theme']=="Y"){?>
						<th width="20%">서브세션제목</th>
						<?}?>

						<?if($s['session_etc_info']=="Y"){?>
						<th width="10%">세션 기타정보</th>
						<?}?>

						<?if($s['category1']=="Y"){?>
						<th width="6%">카테고리 1</th>
						<?}?>
						<?if($s['category2']=="Y"){?>
						<th width="6%">카테고리 2</th>
						<?}?>

						<?if($s['language']=="Y"){?>
						<th width="4%">kor/eng<br/>아이콘표시</th>
						<?}?>
			...
			$sub_row = mysqli_fetch_array($sub_result);
				...
				<tr class="bg" id="<?=$col['sid']?>">
					...
					<?if($s['session_etc_info']=="Y"){?>
					<td><input onchange="changeVal(this,'session_etc_info','<?=$col['sid']?>')" type="text" name="session_etc_info" id="session_etc_info"  value="<?=htmlspecialchars($col['session_etc_info'])?>" /></td>
					<?}?>
...
function view_maker(code,tab) {
	window.open("/data/glance_maker.php?code="+code+"&tab="+tab,"");
}
