...
if($d['status']==2){
	$query="select (select count(*) from voting_result_tbl where val='1' and voting_sid=".$d['sid'].") val1, (select count(*) from voting_result_tbl where val='2' and voting_sid=".$d['sid'].") val2, (select count(*) from voting_result_tbl where val='3' and voting_sid=".$d['sid'].") val3, (select count(*) from voting_result_tbl where val='4' and voting_sid=".$d['sid'].") val4, (select count(*) from voting_result_tbl where val='5' and voting_sid=".$d['sid'].") val5, (select count(*) from voting_result_tbl where val='6' and voting_sid=".$d['sid'].") val6, (select count(*) from voting_result_tbl where val='7' and voting_sid=".$d['sid'].") val7, (select count(*) from voting_result_tbl where val='8' and voting_sid=".$d['sid'].") val8, (select count(*) from voting_result_tbl where voting_sid=".$d['sid'].") val;";
...
<div class="wrapper">
	<input type="hidden" id="start_type" value="<?=$d['start_type']?>">
	<input type="hidden" id="start_txt" value="<?=$d['start_txt']?>">
	<div class="fullScreen">
		...
		<div class="answer">
			...
			<dl class="option <?if(mb_strlen($d['answer1'], 'utf-8')>50){?>ss<?}else if(mb_strlen($d['answer1'], 'utf-8')>25){?> s<?}?>">
				...
				<dd>
					...
					<?if($d['status']==2){?>
					<span><?=$r['val']?round($r['val1']*100/$r['val'],1):'0'?>%</span>
					<?}?>
					</div></div>
				</dd>
			</dl>
			...
			<dl class="option <?if(mb_strlen($d['answer2'], 'utf-8')>50){?>ss<?}else if(mb_strlen($d['answer2'], 'utf-8')>25){?> s<?}?>">
				...
				<dd>
					...
					<?if($d['status']==2){?>
					<span><?=$r['val']?round($r['val2']*100/$r['val'],1):'0'?>%</span>
					<?}?>
					</div></div>
				</dd>
			...
			<dl class="option <?if(mb_strlen($d['answer3'], 'utf-8')>50){?>ss<?}else if(mb_strlen($d['answer3'], 'utf-8')>25){?> s<?}?>">
				...
				<dd>
					...
					<?if($d['status']==2){?>
					<span><?=$r['val']?round($r['val3']*100/$r['val'],1):'0'?>%</span>
					<?}?>
					</div></div>
				</dd>
			...
			<dl class="option <?if(mb_strlen($d['answer4'], 'utf-8')>50){?>ss<?}else if(mb_strlen($d['answer4'], 'utf-8')>25){?> s<?}?>">
				...
				<dd>
					...
					<?if($d['status']==2){?>
					<span><?=$r['val']?round($r['val4']*100/$r['val'],1):'0'?>%</span>
					<?}?>
					</div></div>
				</dd>
			...
			<dl class="option <?if(mb_strlen($d['answer5'], 'utf-8')>50){?>ss<?}else if(mb_strlen($d['answer5'], 'utf-8')>25){?> s<?}?>">
				...
				<dd>
					...
					<?if($d['status']==2){?>
					<span><?=$r['val']?round($r['val5']*100/$r['val'],1):'0'?>%</span>
					<?}?>
					</div></div>
				</dd>
			...
			<dl class="option <?if(mb_strlen($d['answer6'], 'utf-8')>50){?>ss<?}else if(mb_strlen($d['answer6'], 'utf-8')>25){?> s<?}?>">
				...
				<dd>
					...
					<?if($d['status']==2){?>
					<span><?=$r['val']?round($r['val6']*100/$r['val'],1):'0'?>%</span>
					<?}?>
					</div></div>
				</dd>
			...
			<?if($d['answer7']!="" || $d['answer7_img']!=""){?>
			<dl class="option <?if(mb_strlen($d['answer7'], 'utf-8')>50){?>ss<?}else if(mb_strlen($d['answer7'], 'utf-8')>25){?> s<?}?>">

				<dt <?if($d['status']==2 && $d['correct']=="7"){?>style='color:#12a3df'<?}?> class="fSize<?if(mb_strlen($d['answer7'], 'utf-8')>50){?>_ss<?}else if(mb_strlen($d['answer7'], 'utf-8')>0){?>_s<?}?>">⑦ <?=$d['answer7']?></dt>

			
				<dd>
					<div class="graphBar"><div <?if($d['correct']=="7"){echo "class='point'";}?> style="width:<?=$r['val7']*100/$r['val']?>%;">
					<?if($d['status']==2){?>
					<span><?=$r['val']?round($r['val7']*100/$r['val'],1):'0'?>%</span>
					<?}?>
					</div></div>
				</dd>
			...
			<?if($d['answer8']!="" || $d['answer8_img']!=""){?>
			<dl class="option <?if(mb_strlen($d['answer8'], 'utf-8')>50){?>ss<?}else if(mb_strlen($d['answer8'], 'utf-8')>25){?> s<?}?>">

				<dt <?if($d['status']==2 && $d['correct']=="8"){?>style='color:#12a3df'<?}?> class="fSize<?if(mb_strlen($d['answer8'], 'utf-8')>50){?>_ss<?}else if(mb_strlen($d['answer8'], 'utf-8')>0){?>_s<?}?>">⑧ <?=$d['answer8']?></dt>

			
				<dd>
					<div class="graphBar"><div <?if($d['correct']=="8"){echo "class='point'";}?> style="width:<?=$r['val8']*100/$r['val']?>%;">
					<?if($d['status']==2){?>
					<span><?=$r['val']?round($r['val8']*100/$r['val'],1):'0'?>%</span>
					<?}?>
					</div></div>
				</dd>
		...
		<div id="util" class="util">
			<!-- 결과보기 버튼 클릭 후 노출 -->
			<span class="result" style="display: <?if($d['status']!='2'){?>none<?}?>;"><img src="/image/voting_result.png" alt="Voting Result"></span>
		<?if($d['status']!="2"){?>
