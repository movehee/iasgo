<?
	...
	$query = "select * from faculty_category where depth='1' and del='N' order by sort_num asc";
	...
	$(function(){
		$(".sort_table").tableDnD({ 
			...
			onDrop: function(table, row){ 
				...
				$.ajax({
					type:"POST",
					url:"/faculty/fac_sort_change.php",
	...
	<tbody id="product">
		<?
			...
			while(is_array($d=$result->fetchRow(DB_FETCHMODE_ASSOC))){
				$poster_cnt = $conn->getOne("select count(*) from faculty_matching where category='".$d['sid']."'");

				$category_sub_cnt = $conn->getOne("select count(*) from faculty_category where depth='2' and psid='".$d['sid']."' and del='N'");
		?>
		<tr id="<?=$d['sid']?>">
			...
			<td style="padding:0px;border-bottom:0px;border-top:0px;">
				<table class="tblDef tblList sort_table"  style="margin:0px;padding:0px;border:0px;">
					...
					if($category_sub_cnt>0){
						$query_sub = "select * from faculty_category where depth='2' and psid='".$d['sid']."' and del='N' order by sort_num asc";
						...
						while(is_array($sub=$result_sub->fetchRow(DB_FETCHMODE_ASSOC))){
							$category_cnt = $conn->getOne("select count(*) from faculty_matching where category='".$d['sid']."' and category_sub='".$sub['sid']."'");
					?>
					<tr id="<?=$sub['sid']?>">
						<td class="al" style="border-top:0px;border-left:0px;padding-left:10px;"><?=$sub['title']?></td>
						<td style="width:60px;border-top:0px;"><?=number_format($category_cnt)?>개</td>
			...
			<td style="cursor:default;border-top:0px;">
				...
				<img src="/image/icon/icon_del.png" class="hand" onclick="common_delete('<?=$d['sid']?>','faculty_category')">
			</td>
		</tr>
		<?$virtualRecordNo++;}?>
	</tbody>
