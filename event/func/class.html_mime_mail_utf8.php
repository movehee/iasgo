...
function send($to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = ''){
  ...
  if($this->de_headers == ''){
                   $this->de_headers = $this->headers;
  }else{
          $this->headers = $this->de_headers;
  }
                

  if($to_name != '') {
    //$to_name = $this->mail_header_to_utf8($to_name);
    $to = '"'. $this->mail_header_to_utf8($to_name) .'" <'.$to_addr.'>';
  }else {
    $to = $to_addr;
  }
  ...
  if($from_name != '') {
    //$from_name = $this->mail_header_to_utf8($from_name);
    $this->from_chk = '"'. $this->mail_header_to_utf8($from_name) .'" <'.$from_addr.'>';
  }else {
    $this->from_chk = $from_addr;
  }


  $this->headers.= 'From: '.$this->from_chk."\r\n";

  $this->headers.= $headers; 

  $subject = $this->mail_header_to_utf8($subject);

  mail($to, $subject, $this->mime, $this->headers);

  //mail($to, $subject, $this->mime, $this->headers, "-f " . $this->from_chk);
