...
class Mms{
  ...
  function mms_send($PHONE, $CALLBACK, $MSG, $ETC1, $REQDATE, $TYPE) {
    ...
    $query = "INSERT INTO MMS_MSG 
                ...
                (concat(substring('".$MSG."', 1, 10), '...'),'".$PHONE."','".$CALLBACK."','0',".$REQDATE.", '".$MSG."',0,null,'43200', '".$ETC1."', '".$this->group_seq_val."', '".$this->user_id."', '".$this->siteurl."', '".$TYPE."')";
...
*/
  ...
  function mms_query($table_name) {
	...
	if("112.76.194.32" == $_SERVER['REMOTE_ADDR'])
	{
		//echo $query."<<<br/>";
	}
  ...
  function mms_file_cnt($table_name) {
    $this->set_group('', 'B');
  ...
  function mms_file_failcnt($table_name) {
    $this->set_group('', 'G');
  ...
  function set_group($group_seq, $select_con) {
    ...
    if($select_con == 'T'){
      ...
      $this->group_query = " and FILE_CNT=0";
    }elseif($select_con == 'B'){
      $this->group_query = " and FILE_CNT>0";
    }elseif($select_con == 'F'){
      $this->group_query = " and FILE_CNT=0 and (STATUS = '2' or STATUS = '3') and RSLT != '1000' ";
    }elseif($select_con == 'G'){
      $this->group_query = " and FILE_CNT>0 and (STATUS = '2' or STATUS = '3') and RSLT != '1000' ";
    }
  ...
  function rslt_status($status) {
    switch($status){
      ...
      //case '2001' : $result = '주소(포맷) 에러 – 전화번호 유효하지않음,결번,번호이동'; break;
      case '2001' : $result = '주소(포맷) 에러'; break;
      case '2002' : $result = 'Content-length 오류'; break;
      //case '2003' : $result = 'MIME 형식 오류 (컨텐츠 개체, 사이즈, 타입 등의 오류)'; break;
      ...
      //case '3002' : $result = '전송 시간 초과 – 단말기정상수신Report가 4일이내 미도착'; break;
      ...
      //case '4001' : $result = '인증실패–1.G/W인증실패 2.단말기(SKT)제한적인증실패'; break;
