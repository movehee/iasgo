...
class Sms{
  ...
  var $dbName = 'sms';
  var $siteurl = 'neuro.or.kr';
  var $group_seq_val;
  var $group_gubun;
  var $groupBy_query = '';
  var $prev_con = 'N';
  var $time_query = 'sendtime';
  var $user_id = 'webmaster99';
  
  
  

  ### sms 그룹 카운트 체크 성공
  function sms_group_cnt($seq){

    $group_query = " and status ='delivered' ";

    $query = "(select seq from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "' AND group_seq='$seq') ";

    if($this->nowBackup_con == 'Y'){
      $query .= "
      UNION ALL
      (select seq from " . $this->nowBackup_smstbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "' AND group_seq='$seq')
      ";
    }
  ...
  ### sms 총 전송 카운트
  function Tsms_cnt($reserve_flag) {    //reserve_flag : Y(예약), N(즉시)

    $flag_query = $this->flag_wherquery($reserve_flag);

    if($this->nowBackup_con == 'N'){
      $query = "select count(seq) from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' $flag_query ";
      $cnt = $this->conn->getOne($query);
      ...
      $query = "
      (select seq from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' $flag_query GROUP BY group_seq )
      UNION ALL
      (select seq from " . $this->nowBackup_smstbl . " where siteurl = '" . $this->siteurl . "' $flag_query GROUP BY group_seq )
      ";
  ...
  function sms_list($pageNav,$num_per_page,$reserve_flag) {  //reserve_flag : Y(예약), N(즉시)
    ...
    if($this->nowBackup_con == 'N'){
      $query = "select * from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' $flag_query ";
      $query .= " GROUP BY group_seq ORDER BY seq DESC LIMIT " . $pageNav->getFirstRecordInPage() . ", " . $num_per_page;
    }else{
      $query = "
      (select *,'now' from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' $flag_query  GROUP BY group_seq )
      UNION ALL
      (select *,'bac' from " . $this->nowBackup_smstbl . " where siteurl = '" . $this->siteurl . "' $flag_query  GROUP BY group_seq )
      ";
  ...
  function sms_cnt(){
    ...
    if($this->nowBackup_con == 'N'){
      $query = "select count(seq) from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' and status ='delivered'";
      $cnt = $this->conn->getOne($query);
      ...
      $query = "
      (select seq from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' and status ='delivered')
      UNION ALL
...
//echo $query;
  ...
  ### sms db 정보 설정
  function Sms($hostName,$user_Name,$user_userPassword,$dbName,$siteurl,$code) {
...
/*
	...
	$query .= ")";
*/
  function sms_send($receive, $msg, $send, $writer, $reserve_flag, $reserve_date,$group_seq=''){
    $group_seq = $group_seq ? $group_seq : time();
    $receive = str_replace("-","",$receive);
    $send = str_replace("-","",$send);

    $query = "INSERT INTO ". $this->sms_tbl . " (destination, body, originator, callback, siteurl, user_id, code, reserve_flag, reserve_date,group_seq) VALUES (";
    $query .= "'".$receive."',";
    $query .= "'".$msg."',";
