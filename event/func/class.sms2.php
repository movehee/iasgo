...
class Sms{
  ...
  var $send_cnt = 0;
  var $send_type = "B";
  var $send_overpay = array("A"=>25,"B"=>20);
  var $pay_day = 31;
  var $pay_query;
  ...
  function Tsms_cnt($reserve_flag,$groupby_flag) {    //reserve_flag : Y(예약), N(즉시)
    ...
    $query = "(select seq from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'  $flag_query $this->pay_query $this->groupBy_query)";

    if($this->nowBackup_con == 'Y'){ //현재 달의 백업 테이블이 존재 한다면
      $query .= "
        UNION ALL
        (select seq from " . $this->nowBackup_smstbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'  $flag_query $this->pay_query $this->groupBy_query)
      ";
    }
    ...
    if($this->payday_query_con == 'O'){
      $query .= "
        UNION ALL
        (select seq from " . $this->prevBackup_smstbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'  $flag_query $this->pay_query $this->groupBy_query)
      ";
    }
  ...
  function sms_list($pageNav,$num_per_page,$reserve_flag) {  //reserve_flag : Y(예약), N(즉시)
    ...
    $query = "(select *,'now' from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'  $flag_query $this->pay_query $this->groupBy_query)";

    if($this->nowBackup_con == 'Y'){ //현재 달의 백업 테이블이 존재 한다면
      $query .= "
        UNION ALL
        (select *,'bac' from " . $this->nowBackup_smstbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'  $flag_query $this->pay_query $this->groupBy_query)
      ";
    }
    ...
    if($this->payday_query_con == 'O'){
      $query .= "
        UNION ALL
        (select *,'bac' from " . $this->prevBackup_smstbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'  $flag_query $this->pay_query $this->groupBy_query)
      ";
    }
  ...
  function sms_excel_list($reserve_flag) {  //reserve_flag : Y(예약), N(즉시)
    ...
    $query .= "(select *,'now' from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "' $flag_query $this->groupBy_query)";

    if($this->nowBackup_con == 'Y'){
      $query .= "
      UNION ALL
      (select *,'bac' from " . $this->nowBackup_smstbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "' $flag_query $this->groupBy_query)
      ";
    }
    $query .= " ORDER BY seq ";
  ...
  function sms_group_list($pageNav,$num_per_page,$reserve_flag,$group_seq) {  //reserve_flag : Y(예약), N(즉시)
    ...
    $query = "(select *,'now' from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'   $flag_query  and group_seq=$group_seq)";

    if($this->nowBackup_con == 'Y'){
      $query .= "
      UNION ALL
      (select *,'bac' from " . $this->nowBackup_smstbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'   $flag_query  and group_seq=$group_seq)
      ";
    }
...
//$this->col_reserveinfo["body"] = iconv("UTF-8","EUC-KR",$this->col_reserveinfo["body"]);
//echo $this->col_reserveinfo["body"];
//die();
  ...
  ### sms 그룹 카운트 체크 성공
  function sms_group_cnt($group_seq, $select_con){
    ...
    $query = "(select seq from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'  $group_query and group_seq=$group_seq) ";
    ...
    if($this->nowBackup_con == 'Y'){
      $query .= "
      UNION ALL
      (select seq from " . $this->nowBackup_smstbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'  $group_query and group_seq=$group_seq)
      ";
    }
  ...
  function sms_cnt(){
    ...
    $query = "(select seq from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "' and status ='delivered' $this->pay_query)";

    if($this->nowBackup_con == 'Y'){ //현재 달의 백업 테이블이 존재 한다면
      $query .= "
        UNION ALL
        (select seq from " . $this->nowBackup_smstbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'  and status ='delivered' $this->pay_query)
      ";
    }
    ...
    if($this->payday_query_con == 'O'){
      $query .= "
         UNION ALL
        (select seq from " . $this->prevBackup_smstbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "'  and status ='delivered' $this->pay_query) 
      ";
    }
...
//echo $query;
  ...
  function sms_failcnt(){
    ...
    $query = "(select seq from " . $this->sms_tbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "' and status !='delivered')";

    if($this->nowBackup_con == 'Y'){      
      $query .="
       UNION ALL
      (select seq from " . $this->nowBackup_smstbl . " where siteurl = '" . $this->siteurl . "' and user_id = '" . $this->user_id . "' and status !='delivered') 
      ";
    }
  ...
  ### sms db 정보 설정
  function Sms($code,$reserve_flag="N",$user_id, $back_date=null ) {

    $flag_query = $this->flag_wherquery($reserve_flag);
    
    if(!$back_date) $back_date =date('Ym');
    ...
    $this->nowBackup_smstbl .= '_' . $back_date;
