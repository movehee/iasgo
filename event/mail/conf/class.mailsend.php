...
//echo $query;
  ...
  //해당 SEQ값을 기준으로 해당 CODE 정보를 뽑아온다. $coerce_chk값이 1이면 강제로 업로드를 진행한다.
  function get_rcode( $ecare_no, $key, $count, $coerce_chk=0 )
  { 
  	 if(!$this->conn) $this->conn(); 
      $cds = array();
      $query = "select ERROR_CD, CUSTOMER_KEY from dbo.NVECARESENDLOG where ECARE_NO = ".$ecare_no." and CUSTOMER_KEY = '".$key."' ";
      $result= $this->conn->query($query);
      
      if(DB::isError($result)) die("rm3".$result->getMessage());
      $c = $result->fetchRow(DB_FETCHMODE_ASSOC); 
