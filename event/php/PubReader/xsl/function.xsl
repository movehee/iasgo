<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="assign-ref-object-id">
  		<xsl:attribute name="class">fig-table-link <xsl:apply-templates select="@ref-type" /> figpopup</xsl:attribute>
		<xsl:attribute name="href">#<xsl:apply-templates select="@rid" /></xsl:attribute>
		<xsl:attribute name="target"><xsl:value-of select="@ref-type" /></xsl:attribute>
		<xsl:attribute name="rid-figpopup"><xsl:apply-templates select="@rid" /></xsl:attribute>
		<xsl:attribute name="rid-ob">ob-<xsl:apply-templates select="@rid" /></xsl:attribute>		
		<xsl:attribute name="co-legend-rid">lgnd_<xsl:apply-templates select="@rid" /></xsl:attribute>
  </xsl:template>
  ...
  <xsl:template name="assign-img-attr">
  		...
  		<xsl:attribute name="style">max-height:80%</xsl:attribute>
  ...
  <xsl:template name="assign-src-url">
  	...
  	<xsl:choose>
  		<xsl:when test="contains(@xlink:href, '.tif' )"><xsl:value-of select="concat(substring-before(@xlink:href, '.tif'), '.gif')" /></xsl:when>
  ...
  <xsl:template name="assign-img-src" >
	...
	<xsl:if test="/journal-list-url">
		<xsl:element name="a">
			...
			<xsl:attribute name="href"><xsl:apply-templates select="/journal-list-url" /></xsl:attribute>
	...
	<xsl:element name="a">
		<xsl:attribute name="class"><xsl:text>navlink</xsl:text></xsl:attribute>
		<xsl:attribute name="href"><xsl:apply-templates select="/journal-issues-url" /></xsl:attribute>		
		<xsl:apply-templates select="/custom-abbr" />
	</xsl:element>		
	
	<xsl:element name="a">
		<xsl:attribute name="href"><xsl:apply-templates select="/journal-articles-url" /></xsl:attribute>	
		<xsl:apply-templates select="article/front/article-meta" mode="article-no"/>		
	</xsl:element>
...
<xsl:template name="alternative-link">
		<xsl:element name="a">
			<xsl:attribute name="href"><xsl:apply-templates select="/article-url" /></xsl:attribute>
			<xsl:attribute name="target"><xsl:text>_blank</xsl:text></xsl:attribute>
			<xsl:text>Classic View</xsl:text>
		</xsl:element>
		<xsl:element name="a">
			<xsl:attribute name="href"><xsl:apply-templates select="/pdf-file" /></xsl:attribute>
			<xsl:attribute name="target"><xsl:text>_blank</xsl:text></xsl:attribute>
			<xsl:text>Article PDF</xsl:text>
		</xsl:element>
		<xsl:if test="//article-id[@pub-id-type='doi']">
			<xsl:element name="a">
				<xsl:attribute name="href"><xsl:apply-templates select="(//article-id[@pub-id-type='doi'])[1]" mode="href-link" /></xsl:attribute>
				<xsl:attribute name="target"><xsl:text>_blank</xsl:text></xsl:attribute>
...
<xsl:template name="format-date">
  ...
  <xsl:template name="name-link-href">
  	<xsl:attribute name="href">advanced/?term=author&amp;given_name=<xsl:apply-templates select="given-names" />&amp;surname=<xsl:apply-templates select="surname" /></xsl:attribute>
  </xsl:template>
  
  <xsl:template name="keyword-link-href">
  	<xsl:attribute name="href">advanced/?term=keywords like &apos;%<xsl:apply-templates select="." />%&apos;</xsl:attribute>
  </xsl:template>
  ...
  <xsl:template name="print-author">
  	...
  	<xsl:apply-templates select="ancestor::contrib/*[not(self::name or self::name-alternatives)]" />
  </xsl:template>
