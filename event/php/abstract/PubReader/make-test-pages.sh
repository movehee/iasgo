...
for testfile in `ls ../upload/dcl_xml/*.xml`
do
  filename=${testfile%.*}
  echo "Converting $testfile to $filename.html"
  java -jar $SAXON9 -xsl:nlm2pmc.xsl -s:$testfile > $filename.xml.xml
  java -jar $SAXON9 -xsl:test-page.xsl -s:$filename.xml.xml > $filename.html
  rm -f $filename.xml.xml
done
