  ...
  exclude-result-prefixes="xlink">
  	...
  	<xsl:template name="load-img-by-id">
		<xsl:element name="img">
			...
			<xsl:attribute name="src"><xsl:apply-templates select="//thumb-path" /><xsl:value-of select="@id" />.gif</xsl:attribute>
			<xsl:attribute name="src-large"><xsl:apply-templates select="//thumb-path" /><xsl:value-of select="@id" />.gif</xsl:attribute>
		</xsl:element>
  	</xsl:template>
