...
if($abs_setting_col['abs_sync']=='Y') { /*학회사이트 데이터를 사용할때*/
	...
	<?
	}else if($abs_setting_col['abs_top_menu']==2){
		if(!$top_tab) $top_tab = 1;
	?>
	<ul class="tabMenu">
		<li class="menu <?if($top_tab=='1'){?>on<?}?>"><a href="?code=<?=$code?>&deviceid=<?=$deviceid?>&top_tab=1"><?=$abs_setting_col['abs_top_menu_txt1']?></a></li>
		<li class="menu <?if($top_tab=='2'){?>on<?}?>"><a href="?code=<?=$code?>&deviceid=<?=$deviceid?>&top_tab=2"><?=$abs_setting_col['abs_top_menu_txt2']?></a></li>
	</ul>
	<?}?>
	...
	<?
	if($abs_setting_col['abs_top_menu']=='2' && $top_tab=='2') {

		if(is_file($_SERVER['DOCUMENT_ROOT'] . "voting/config/".$code.".php")) include $_SERVER['DOCUMENT_ROOT'] . "voting/config/".$code.".php";

		foreach($_SET['abstract_category'] as $top_key => $top_val) {
	?>
		<li><a href="./list.php?code=<?=$code?>&deviceid=<?=$deviceid?>&parent=<?=$col['sid']?>&cat=<?=$top_key?>"><?=$top_val?></a></li>
	<?
		}
		...
		if($cnt_row['cnt']==0){
		?>	
			<li><a href="./list.php?code=<?=$code?>&deviceid=<?=$deviceid?>&parent=<?=$col['sid']?>&tab=<?=$tab?>"><?=$col['info']?></a></li>
		<?}else{?>
			<li><a href="./category.php?code=<?=$code?>&deviceid=<?=$deviceid?>&parent=<?=$col['sid']?>&tab=<?=$tab?>"><?=$col['info']?></a></li>
		<?}}?>
...
<?}else if($code=='knpa2019f'){?>
			...
			<a href="http://ezv.kr/php/abstract/knpa2019f/knpa2019f_abs.pdf" target="_blank">
			<img src="http://ezv.kr/php/abstract/knpa2019f/knpa2019f.png"></a>
