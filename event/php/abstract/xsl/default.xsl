<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="main-title"
    ...
    <xsl:call-template name="print-lngd-in-content" />
  </xsl:template>
...
<xsl:template match="sec-meta">
  ...
  <xsl:template match="*[@content-type]/xref">
    <xsl:apply-templates/>    
  </xsl:template>

  <xsl:template match="contrib/xref | xref[@ref-type='fn']">	
	<a href="#{@rid}"><xsl:choose>
  		<xsl:when test="not(self::sup)">
  ...
  <xsl:template match="list[@list-type='bullet' or not(@list-type)]" mode="list">
	...
	<xsl:call-template name="print-lngd-in-descendant" />
  </xsl:template>
