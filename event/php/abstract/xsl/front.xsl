...
<!--   		<xsl:apply-templates select="journal-meta" mode="metadata" />		  	 -->
  ...
  <xsl:template match="journal-meta" mode="metadata" >
   ...
   <xsl:template match="book-part-meta" mode="metadata">

    <h1><xsl:value-of select="./title-group/title"/></h1>
    <xsl:if test="./title-group/trans-title-group/trans-title">
      <h3><xsl:value-of select="./title-group/trans-title-group/trans-title"/></h3>
    </xsl:if>
