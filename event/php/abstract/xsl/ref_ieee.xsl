...
<xsl:template match="ref-list" mode="back-ref-list">
	...
	<xsl:template match="conf-loc" mode="ref">
		...
		<xsl:choose>			
			...
			<!-- 그외 일 경우 -->
			<xsl:otherwise>
				<xsl:apply-templates/><xsl:apply-templates select="." mode="ref-period" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>

	<xsl:template match="article-title" mode="ref">
		<xsl:apply-templates/>
		<xsl:text> </xsl:text>	
	</xsl:template>
	...
	<xsl:template match="fpage" mode="ref">		
		...
		<xsl:choose>						
			<xsl:when test="following-sibling::*[1] and not(following-sibling::*[1][name()='lpage'])">    
				<xsl:text>, </xsl:text>
			</xsl:when>
			<xsl:when test="following-sibling::*[1][name()='lpage']">    
				<xsl:text></xsl:text>
			</xsl:when>
			<!-- 그외 일 경우 -->
			<xsl:otherwise>
				<xsl:apply-templates select="." mode="ref-period" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="month" mode="ref">	
		<xsl:apply-templates/>	
		<xsl:text> </xsl:text>
	</xsl:template>
	...
	<xsl:template match="day" mode="ref">	
		<xsl:apply-templates/>
		<xsl:text> </xsl:text>											
	</xsl:template>
	...
	<xsl:template match="source" mode="ref">
		<xsl:apply-templates/>		
		<xsl:choose>	
			<xsl:when test="../@publication-type='book' and (following-sibling::*[1][name()='publisher-name'])">    
				<xsl:text> </xsl:text>
			</xsl:when>
			<xsl:when test="../@publication-type='book'">    
				<xsl:text>, </xsl:text>
			</xsl:when>
			<xsl:when test="../@publication-type='journal' and (following-sibling::*[1][name()='year'])">    
				<xsl:text> </xsl:text>
			</xsl:when>
			<xsl:when test="../@publication-type='web' and (following-sibling::*[1][name()='volume']) ">    
				<xsl:text>, </xsl:text>
			</xsl:when>
	...
	<xsl:template match="person-group" mode="ref">		
		...
		<xsl:for-each select="name | etal | collab">
			...
			<xsl:if test="position()=last() and count(../name) > 1"> and </xsl:if>
			

			<xsl:apply-templates select="." mode="ref-name"/>		

			<xsl:if test="name()='name' and position()&lt; ($name-count - 1)"><xsl:text>, </xsl:text></xsl:if>
	...
	<xsl:template match="*" mode="ref-period">
		<xsl:choose>
			<xsl:when test="following-sibling::*[1]"> 
				<xsl:call-template name="ref-end2">
	...
	<!-- 참고문헌의 이름의 표시 템플릿 -->
	<xsl:template match="person-group/name" mode="ref-name">				
		<xsl:if test="prefix">
			<xsl:value-of select="prefix"/><xsl:text> </xsl:text>
		</xsl:if><!-- Mr 나 Ms 표시 -->
		<xsl:value-of select="given-names"/><xsl:text> </xsl:text><xsl:value-of select="surname"/><!-- 성 -->
