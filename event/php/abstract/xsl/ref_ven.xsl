...
<xsl:template match="article-meta" mode="article-no">  
	...
	<xsl:choose>
		<xsl:when test="elocation-id">
			<xsl:text>.</xsl:text><xsl:apply-templates select="elocation-id" />
		</xsl:when>
		<xsl:otherwise>
			<xsl:text>(</xsl:text><xsl:apply-templates select="issue" /><xsl:text>)</xsl:text>
		</xsl:otherwise>
	</xsl:choose>	
</xsl:template>
...
<xsl:template match="ref-list" mode="back-ref-list">
	...
	<xsl:template match="*" mode="ref">		
		<xsl:apply-templates select="." />
		<xsl:call-template name="print-period">
	...
	<xsl:template match="collab" >			
	 ...
	 <!-- 레퍼런스 전용 태그 처리 -->
	<xsl:template match="pub-id" mode="ref">		
