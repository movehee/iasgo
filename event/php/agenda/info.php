...
<div class="session" id="session<?=$col['sid']?>">
	...
	<?if($tab != "-7"){?>
		...
		<?if($setting_col['session_question']=="1" && $col['qna_viewYN']=='Y'){ 
			$q_link = "sid=".$col['sid'];
		?>
		<li style="margin-top:10px;float:right;"><a href="app_question.php?<?=$q_link?>" class="btnQuestion" style="margin-top:-15px;<?if($code=='NST2020'){?>font-size:19px !important;<?}?>"><?=$setting_col['question_icon_txt']?></a></li>
		<?}?>
	...
	<div id="session<?=$col['sid']?>_sub" class="toggleCon" <?if($glance){?>style="display:block"<?}?>>
		...
		?>
			...
			<a href="/upload/session/<?=$subcol['lecture_file']?>">
