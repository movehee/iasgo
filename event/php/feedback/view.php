...
if($code=='m2019s' || $code=='m2019f' || $code=='kaim5') {
	$fixed_btn = true;
}
...
<div class="wrapper">
	...
	<?if($setting_col['feedback_end_time']>0 &&time()>$setting_col['feedback_end_time']){?>
		<?if($title || $code == "koa2020s"){?>
		<div class="titArea">
			
			<h2><?=empty($title)? strtoupper($setting_col['feedback_txt']) :  strtoupper($setting_col['feedback_txt'])?></h2>
			<p class="fixedBtn">
	...
	<div <?if($fixed_btn){?>id ="fixedTop2"<?}?> <?if($fixed_btn){?> style="position: fixed; left:0; top:0; width:100%"<?}?>>
	<?if($title || $code == "koa2020s"){?>
		<div class="titArea">
	
			<h2 class="    text-transform: capitalize;"><?=empty($title)? strtoupper($setting_col['feedback_txt']) :  strtoupper($setting_col['feedback_txt'])?></h2>
			<p class="fixedBtn">
				<a href="back.php" class="back"><i class="fas fa-arrow-left" title="이전"></i></a>
			</p>
	...
	<input type="hidden" name="regist_sid" id="regist_sid" value="<?=$regist_sid?>">
...
-->
			...
			<?if($col['type']=="41"){?>
			<div class="div_<?=$col['parent']?>_<?=$col['parent_val']?>" <?if($col['parent'] && $val_list[$col['parent']] != $col['parent_val']){?>style="display:none"<?}?>>
				<p class="border"><textarea name="memo<?=$q?>"  placeholder="<?=$string['feedback_placeholder']?>" id="memo<?=$q?>" cols="30" rows="10"><?=$d['memo'.$q]?></textarea></p>
				
				
				<input type="hidden" id="answer<?=$j?>" ans-type="memo" value="<?=$q?>">
				<input type="hidden" id="answer_necessary<?=$j?>" value="<?=$col['necessary']?>">
				<input type="hidden" id="answer_necessary_txt<?=$j?>" value="<?=$col['necessary_txt']?>">
				<?$q++;?>
				<?$j++;?>
			</div>
			...
			<div class="div_<?=$col['parent']?>_<?=$col['parent_val']?>" <?if($col['parent'] && $val_list[$col['parent']] != $col['parent_val']){?>style="display:none"<?}?>>
				<?
					$val1 = $col['val1'];
					$val1 = str_replace("{mobile}", $_COOKIE['mobile'], $val1);
					$val1 = str_replace("{name}", $_COOKIE['name'], $val1);
					echo $val1;
				?>
	...
	<div class="btn fullBtn" <?if($fixed_btn){?>style="position: fixed;left: 5%;bottom: 0;width: 90%;padding:20px 0px 20px 0px !important;"<?}?>><input type="submit" value="<?=$setting_col['feedback_send']?>" class="btnPoint"></div>
	</form>
	</div>
	<?}?>
	...
	<?
	if($d['event_yn'] == 'Y'){
		include_once $_SERVER['DOCUMENT_ROOT']."voting/php/feedback/event/".$code.".php";
	}	
	?>
</div>
...
jQuery(function($) {
	...
	$('#researchF').submit(function() {
		...
		for(i=1;i<$("#answer_cnt").val();i++)
		{
			...
			var ans_type = $("#answer"+i).attr("ans-type");
			if(ans_type == "memo") {
				q = $("#answer"+i).val();
				if($("textarea#memo"+q).val()=="" && document.getElementById("answer_necessary"+i).value=="Y") {
					alert($("#answer_necessary_txt"+i).val());
					$("textarea#memo"+q).focus();
					return false;
				}
			} else {
