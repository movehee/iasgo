	...
	<fieldset style="border:1px solid <?=$css_col['feedback_bg']?>">
		...
		?>
			...
			<div id="div_<?=$col['parent']?>_<?=$col['parent_val']?>" <?if($col['parent'] && $val_list[$col['parent']] != $col['parent_val']){?>style="display:none"<?}?>>
				<h2 class="subTitBg" id="tit<?=$col['sid']?>"><span><b><?=$col['val1']?></b></span><?=$col['val2']?></h2>
			<div>
			<?}?>
			...
			<?if($col['type']=="1"){?>
			<div id="div_<?=$col['parent']?>_<?=$col['parent_val']?>" <?if($col['parent'] && $val_list[$col['parent']] != $col['parent_val']){?>style="display:none"<?}?>>
				<h2 class="subTitBg" id="tit<?=$col['sid']?>" style="padding: 10px 15px 10px 15px;"><?=$col['val1']?></h2>
			</div>
			<?}?>
			...
			<div id="div_<?=$col['parent']?>_<?=$col['parent_val']?>" <?if($col['parent'] && $val_list[$col['parent']] != $col['parent_val']){?>style="display:none"<?}?>>
				<dl class='feedbackItem'>
					...
					<?for($i=1;$i<=$col['cnt'];$i++){?>
						...
						<span class="changeBg inputR <?if ($d['answer'.$j]==$i){?> on<?}?>" id="q<?=$j?>_<?=$i?>" sid="<?=$col['sid']?>" onclick="chk_qu(<?=$j?>,<?=$i?>,<?=$col['cnt']?>,<?=$col['sid']?>)">
			...
			<div id="div_<?=$col['parent']?>_<?=$col['parent_val']?>" <?if($col['parent'] && $val_list[$col['parent']] != $col['parent_val']){?>style="display:none"<?}?>>
				<ul class='feedbackItem'>
					...
					<?for($i=1;$i<=$col['cnt'];$i++){?>
						<li sid="<?=$col['sid']?>" onclick="chk_qu(<?=$j?>,<?=$i?>,<?=$col['cnt']?>,<?=$col['sid']?>)">
			...
			<div id="div_<?=$col['parent']?>_<?=$col['parent_val']?>" <?if($col['parent'] && $val_list[$col['parent']] != $col['parent_val']){?>style="display:none"<?}?>>
				<dl class='feedbackItem'>
					...
					<?for($i=1;$i<=$col['cnt'];$i++){?>
						...
						<span class="changeBg2 inputR <?if ($d['answer'.$j]==$i){?> on<?}?>" id="q<?=$j?>_<?=$i?>" sid="<?=$col['sid']?>" onclick="chk_qu(<?=$j?>,<?=$i?>,<?=$col['cnt']?>,<?=$col['sid']?>)">
						<i class="fas fa-check"></i></span><label sid="<?=$col['sid']?>" onclick="chk_qu(<?=$j?>,<?=$i?>,<?=$col['cnt']?>,<?=$col['sid']?>)" for="q<?=$j?>_<?=$i?>"><?=$col['sub'.$i]?></label>
					<?}?>
					<input type="hidden" name="parent<?=$j?>" id="parent<?=$j?>" value="<?=$col['parent']?>">
					<input type="hidden" name="parent_val<?=$j?>" id="parent_val<?=$j?>" value="<?=$col['parent_val']?>">
					<input type="hidden" name="answers<?=$col['sid']?>" id="answers<?=$col['sid']?>" value="<?=$d['answer'.$j]?>">
				...
				<dl>
					<dd>
						<ul class="changeBg">
							...
							<li><a id="q<?=$j?>_<?=$i?>" class="<?if ($d['answer'.$j]==$i){?> on<?}?>" sid="<?=$col['sid']?>" onclick="chk_qu(<?=$j?>,<?=$i?>,'5',<?=$col['sid']?>)"><?=$i?></a></li>
							<?}?>
							<input type="hidden" name="parent<?=$j?>" id="parent<?=$j?>" value="<?=$col['parent']?>">
							<input type="hidden" name="parent_val<?=$j?>" id="parent_val<?=$j?>" value="<?=$col['parent_val']?>">
							<input type="hidden" name="answers<?=$col['sid']?>" id="answers<?=$col['sid']?>" value="<?=$d['answer'.$j]?>">
				...
				<dl>
					<dd>
						...
						<?for($i=1;$i<=$col['cnt'];$i++){?>
							...
							<li sid="<?=$col['sid']?>" onclick="chk_qu(<?=$j?>,<?=$i?>,<?=$col['cnt']?>,<?=$col['sid']?>)" style="width:<?=100/$col['cnt']?>%">
...
<!--
							<li sid="<?=$col['sid']?>" onclick="chk_qu(<?=$j?>,<?=$i?>,<?=$col['cnt']?>)">
						<span class="changeBg inputR <?if ($d['answer'.$j]==$i){?> on<?}?>" id="q<?=$j?>_<?=$i?>" >
						<i class="fas fa-check"></i></span><label for=""><?=$col['sub'.$i]?></label>
