...
<div class="intro">
	...
	-->
	<div class="btnArea">
		<form id="frm" name="frm" action="./stroke200627_proc.php" method="post">
			...
			<fieldset>
				...
				<?}?>
				<?if($event_col['mobileYN']=="Y"){?>
				<input type="text" name="mobile" id="mobile" placeholder="휴대폰번호를 입력해 주세요">
				<?}?>
				
				<p class="btn"><a onclick="frm_submit('<?=$event_col['nameYN']?>','<?=$event_col['officeYN']?>','<?=$event_col['emailYN']?>','<?=$event_col['licenseYN']?>','<?=$event_col['mobileYN']?>','<?=($event_col['agreeYN']=='Y'||$event_col['agree_message'])?'Y':'';?>');" class="btnDef">START</a></p>
			</fieldset>
		</form>
...
<script>
	...
	function frm_submit(a1,a2,a3,a4,a5,a6){
		...
		if(a5 && !document.getElementById("mobile").value){
			alert("휴대폰번호를 입력해 주세요.");
			return false;
		}
		if(a6 && !document.getElementById("agree").value){
