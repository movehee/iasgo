<?
$event_query="SELECT * FROM event_tbl where code='".$code."'";
$event_result = mysqli_query($conn, $event_query);
$event_col = mysqli_fetch_array($event_result);
...
<?if($setting_col['regist_top_text']){?><div><?=$setting_col['regist_top_text']?></div>
<?}else{?>
<div id="lnb" class="ac padding0 fwBold" style="background-color:#006771">
	<span style="color:#fff; font-size:16px; letter-spacing:1.8px; line-height:2.3;"><?=$event_col['name']?></span>
</div>
<?}?>
...
<form enctype="multipart/form-data" name="registform" id="registform" action="./post.php" method="post"> 
	<table class="registList">	
		...
		<tr class="parent_<?=$regist_d['parent']?>_<?=$regist_d['sid']?> <?if($regist_d['countrychk']=="1"){?>kor<?}else if($regist_d['countrychk']=="2"){?>eng<?}else if($regist_d['countrychk']=="3"){?>count_sel<?}?>" style="<?=$style_txt?>" <?if($regist_d['parent']){?>class="parent<?=$regist_d['parent']?> <?=$regist_d['parent_val']?>"<?}?>>
			<th class="regist_th" title="<?=$regist_d['info']?>"><?=$regist_d['info']?><?if($regist_d['necessary']=="Y"){?> <span class="necessary">*</span><?}?></th>
			<td class="regist_td <?if($regist_d['type']=="30"){?>small<?}?>">
				...
				<?
					...
					if(is_null($email_array) || in_array($email_array['1'], $config['email'])) {
						$email_etc_dis = "none";
					}
				?>
					<input class="email" type="text" name="val_<?=$regist_d['info_orderby']?>[0]" id="val_<?=$regist_d['info_orderby']?>"  value="<?=$email_array['0']?>" />@
					
					<select style="width:40%" id="sel_email">
						<option value="">선택</option>
						<?foreach($config['email'] as $mkey){?>
						<option value="<?=$mkey?>" <?if($email_array['1']==$mkey){?>selected<?}?> ><?=$mkey?></option>
						<?}?>
						<option value="self">직접입력</option>
					</select>
					<input class="email email_etc" type="text" name="val_<?=$regist_d['info_orderby']?>[1]" id="val_<?=$regist_d['info_orderby']?>" style="margin-top: 5px;display:<?=$email_etc_dis?>"  value="<?=$email_array['1']?>" />
				...
				<?}else if($regist_d['type']=="120"){?>

				<?
					$name_array=null;
					if($d['info'.$regist_d['info_orderby']]){
						$name_array = split("&&",$d['info'.$regist_d['info_orderby']]);
					}
				?>
					<input class="textW30" type="text" name="val_<?=$regist_d['info_orderby']?>[0]" id="val_<?=$regist_d['info_orderby']?>" style="text-transform: capitalize;" placeholder="First Name" value="<?=$name_array['0']?>" />
					<input class="textW30" type="text" name="val_<?=$regist_d['info_orderby']?>[1]" id="val_<?=$regist_d['info_orderby']?>" style="text-transform: capitalize;" placeholder="Last Name" value="<?=$name_array['1']?>" />
				<?}else if($regist_d['type']=="130"){?>
					<input type="text" class="textW30" name="val_<?=$regist_d['info_orderby']?>" id="val_<?=$regist_d['info_orderby']?>"  value="<?=$d['info'.$regist_d['info_orderby']]?>" onkeypress="onlyNumber();" />
				...
				<?
					...
					<?if($val_type=="1"){
						...
						$info_val_arr = explode("&&", $d['info'.$regist_d['info_orderby']]);
						$info_val = $info_val_arr[0];
						$info_etc_val = $info_val_arr[1];
					?>
					
						<input class="<?if($type_d['countrychk']=="1"){?>kor<?}else if($type_d['countrychk']=="2"){?>eng<?}?><?if($type_d['etc']=='Y'){?> etc<?}?>" <?if($onchangechk){?>onchange="money_change('<?=$regist_d['info_orderby']?>')"<?}?> <?if($info_val==$type_d['sid']){?>checked<?}?>  type='radio' name='val_<?=$regist_d['info_orderby']?>' id='val_<?=$regist_d['info_orderby']?>_<?=$type_d['sid']?>' value="<?=$type_d['sid']?>"/><span  class="<?if($type_d['countrychk']=="1"){?>kor<?}else if($type_d['countrychk']=="2"){?>eng<?}?>"><label for='val_<?=$regist_d['info_orderby']?>_<?=$type_d['sid']?>'><?=$type_d['info']?></label></span>
						
						<?if($type_d['etc']=='Y'){
							if($sid && $type_d['sid']==$info_val){ $etc_dis = "block;"; } else { $etc_dis = "none;"; }
						?>
						<input type="text" name="val_etc_<?=$regist_d['info_orderby']?>" class="text_etc_<?=$regist_d['info_orderby']?>" id="text_etc_<?=$type_d['sid']?>" style="margin-top: 5px;display:<?=$etc_dis?>; width:30%" value="<?=$info_etc_val?>">
						<?}?>
					<?}else if($val_type=="2"){
						$temp = split(",",$d['info'.$regist_d['info_orderby']]);
					...
					?>
						...
						<input class="<?if($type_d['countrychk']=="1"){?>kor<?}else if($type_d['countrychk']=="2"){?>eng<?}?>"  <?if($onchangechk){?>onchange="money_change('<?=$regist_d['info_orderby']?>')"<?}?> type='checkbox' name='val_<?=$regist_d['info_orderby']?>[]' id='val_<?=$regist_d['info_orderby']?>_<?=$type_d['sid']?>' value="<?=$type_d['sid']?>"  <?
						...
						?>/><span  class="<?if($type_d['countrychk']=="1"){?>kor<?}else if($type_d['countrychk']=="2"){?>eng<?}?>"><label for='val_<?=$regist_d['info_orderby']?>_<?=$type_d['sid']?>'><?=$type_d['info']?></label></span>
					<?}else if($val_type=="3"){?>
						<option class="<?if($type_d['countrychk']=="1"){?>kor<?}else if($type_d['countrychk']=="2"){?>eng<?}?>"  <?if($d['info'.$regist_d['info_orderby']]==$type_d['sid']){?>selected<?}?> value="<?=$type_d['sid']?>"><?=$type_d['info']?></option>
					<?}
					$k++;
					}?>
					<?if($val_type=="3"){?>
