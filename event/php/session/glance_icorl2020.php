...
?>
	...
	while(is_array($d = mysqli_fetch_array($result))){
		...
		else if($d['sid'] == '14231') {//OTPX
			$time_height = 312;
			$top = 727;
		}
		else if($d['sid'] == '14233') {//RHPX
			$time_height = 312;
			$top = 1039;
		}


		else if($d['sid'] == '15118') {//OT Video
			$time_height = 433;
			$top = 51;
		}
		else if($d['sid'] == '15119') {//RH Video
			$time_height = 433;
			$top = 484;
		}
		else if($d['sid'] == '15120') {//HN Video
			$time_height = 433;
			$top = 917;
		}
		...
		else if($d['sid'] == '14317') {//포스터 두결부
			$time_height = 407;
			$top = 50;
		}
		else if($d['sid'] == '14318') {//포스터 두결부
			$time_height = 407;
			$top = 458;
		}
		else if($d['sid'] == '14319') {//포스터 두결부
			$time_height = 407;
			$top = 866;
		}


		else if($d['sid'] == '15097') {//OT Video
			$time_height = 407;
		}
		else if($d['sid'] == '15103') {//RH Video
			$time_height = 407;
			$top = 457;
		}
		else if($d['sid'] == '15104') {//HN Video
			$time_height = 407;
			$top = 864;
		}
	...
	<?if($d['liveYN']=='Y'){?>
	<span style="<?if($css_col['session_bullet_eng']){?>background-color:<?=$css_col['session_bullet_eng']?> !important;<?}?> <?if($css_col['session_category_radius']){?>border-radius:<?=$css_col['session_category_radius']?>px;<?}?>" class="bullet<?=$setting_col['bullet_type']?> eng">live</span>
	<?}?>
