...
?>
<div id="background_liner" style="position:absolute;height:<?=$b_height?>px;width:<?=$total_width-2?>px;border:1px solid #dbdbdb;"></div>
...
?>
	...
	for ($i = $min_time; $i < $max_time; $i++) {?>
		<table class="glance_time"  style="height:<?=$height+1?>px;min-height:<?=$height+1?>px;max-height:<?=$height+1?>px;font-size:<?=$session_font_size?>px;max-width:<?=$def_width-$time_width?>px;min-width:<?=$def_width-$time_width?>px;width:<?=$def_width-$time_width?>px; top: <?=($i*60-$startTime)*$height/60+$height2-1?>px; left: 0px;"><tr><td><?=$i?>:00</td></tr></table>
	...
	<?}while(is_array($d = mysqli_fetch_array($result))){
		...
		$top = ($temp2[0]*60 + $temp2[1] - $startTime) * $height / 60 + $height2;
