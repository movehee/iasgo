...
<script>
	$(document).ready(function() {
		...
		$(".glance_time").each(function() {
			$height += $(this).height()+1;
		});
		
		$t_height = $height + ($height2*1 - 2);
		$("#background_liner").css("height", $t_height);
	});
</script>
...
<?
	...
	$room_query="SELECT count(*) cnt FROM session_room_tbl  WHERE del='N' and viewYN='Y' and tab='".$tab."' and add_room='0' and code='".$code."'";
...
?>
	...
	<?
	if($_SET['etc_info'] && $d['etc_info']) { //config.php 에서 설정한 값이 있을경우 아이콘형태로 보여줌
	...
	$glance_view_style = str_replace("{s_etc_info}", $d['session_etc_info'], $glance_view_style);
