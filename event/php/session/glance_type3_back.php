...
<?
	...
	$before ="";
?>
	<div class="glance_room" style="height:<?=$height2?>px; width:<?=$def_width-$time_width?>px; font-size:<?=$session_font_size?>px; top: 0px; left: 0px;line-height:<?=$height2?>px;"></div>

	<?
	$i=0;
	$room_query="SELECT a.sid, a.name,count(b.sid) cnt,a.orderby FROM session_room_tbl a left join session_tbl b on b.room = a.sid and b.tab='".$tab."' WHERE a.code='".$code."' and a.tab='".$tab."' and a.del='N' and a.add_room='0' group by a.sid order by a.orderby asc";
	...
	?>
		...
		<div class="glance_room" style="height:<?=$height2-1?>px;line-height:<?=$height2-1?>px;font-size:<?=$session_font_size?>px; width:<?=$def_width?>px; top: 0px; left: <?=$i*$def_width-$time_width?>px;"><?=$room_d['name']?></div>
		<?}?>
	...
	?>
	<table class="glance_time" style="height:<?=$time_height+1?>px; width:<?=$def_width-$time_width+1?>px;font-size:<?=$session_font_size?>px; top: <?=$top?>px; left: 0px;"><tr><td><?if($time_height>80){?><?=str_replace("-","<br>-<br>",$time_d['time'])?><?}else{?><?=$time_d['time']?><?}?></td></tr></table>
	...
	<?}while(is_array($d = mysqli_fetch_array($result))){
		...
		$time_height = round(($temp3[0]*60 + $temp3[1] - $temp2[0]*60 - $temp2[1] - $break_margin2) * $height / 60)+1;
	...
	<?if($setting_col['glance_view_type']>=50){?>
		<?=$d['category2']?>
	<?}else if($setting_col['glance_view_type']>=40){?>
		<?=$c_row['info']?>
	<?}else if($setting_col['glance_view_type']>=30){?>
		<?=$c_row['info']?> <?=$d['category2']?>
	<?}?>
