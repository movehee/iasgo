...
?>
	...
	while(is_array($time_d = mysqli_fetch_array($time_result))){
		...
		$temp3 = split(":",$temp[1]);
		if($temp2[0]<$min_time){
			$min_time = (int)$temp2[0];
		}

		if($temp3[1]>0){
			$temp3[0] = $temp3[0]+($temp3[1]/60);
		}
		if($temp3[0]>$max_time){
			$max_time = $temp3[0];
			$max_time_sid = $time_d['sid'];
		}
	...
	for ($i = $min_time; $i < $max_time; $i+=0.5) {?>
		<table class="glance_time"  style="height:<?=$height+1?>px;min-height:<?=$height+1?>px;max-height:<?=$height+1?>px;font-size:<?=$session_font_size?>px;max-width:<?=$def_width-$time_width?>px;min-width:<?=$def_width-$time_width?>px;width:<?=$def_width-$time_width?>px; top: <?=($i*60-$startTime)*$height/30+$height2-1?>px; left: 0px;"><tr><td style="vertical-align: top; padding-top: 5px; ">
		
		<?=($i==(int)$i)?$i.":00":floor($i).":30"?><!--<br>-<br><?=(($i+0.5)==(int)($i+0.5))?($i+0.5).":00":floor(($i+0.5)).":30"?>-->
		</td></tr></table>
	...
	while(is_array($d = mysqli_fetch_array($result))){
		...
		$top = ($temp2[0]*60 + $temp2[1] - $startTime) * $height / 30 + $height2;
		...
		$time_height = ($temp3[0]*60 + $temp3[1] - $temp2[0]*60 - $temp2[1] ) * $height / 30;
	...
	" class="glance_session <?if($max_time_sid == $d['time']){?>last_time<?}?>"
	>
	<tr><td style="word-break:keep-all;">
	
	<?if($d['language'] =="1"){?>
		<span class="bullet eng">ENG</span>
	<?}else if($d['language'] =="2"){?>
		<span class="bullet kor">KOR</span>
	<?}?>
...
<script>
	$(document).ready(function() {
		...
		//
		//121-41
		var $fisrt = $(".glance_time").first();
		$fisrt.css('min-height', 41).css('height', 41);
		$(".glance_time").not($fisrt).each(function() {
			h = parseInt($(this).css("top"))-80;
			$(this).css("top", h);
			
		});

		$(".glance_session").not($fisrt).each(function() {
			h = parseInt($(this).css("top"))-80;
			$(this).css("top", h);
		...
		if($(".last_time").length) {
			...
			$(".last_time").each(function() {
				if(t_height < $(".last_time").height()) {
					t_height = $(".last_time").height();
				}
			});

			t_height += 1;

			$(".glance_time").last().css('min-height', t_height).css('height', t_height);
		}
