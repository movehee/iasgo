...
<div class="session" id="session<?=$col['sid']?>">
	...
	<?if($tab != "-7"){?>
		...
		<? if( stristr( $code, "stroke" ) == true ){ //웹심포지엄에서 피드백 컨트롤용으로 사용한다. ?> 
		<li><a href="#" class="btnFeedback <?=$col['feedback_yn']=='Y'?'feedon':''?>" data="<?=$col['sid']?>"><?=$col['feedback_yn']=='N'?'Feedback On':'Feedback Off'?></a></li>
		<? } ?>
		...
		<li style="margin-top:30px;float:right;padding:0 !important;">
		...
		?>
			<?if($live_url) {?>
			<a href="live.php?url=<?=urlencode($live_url)?>" style="vertical-align:top;">
			<img src="/image/icon_live.png" alt="" style="height:26px;">
			</a>
			<?}?>
		<?}?>
		...
		?>
		<a href="app_question.php?<?=$q_link?>" class="btnQuestion" style="<?if($code=='kses190818' || $code=='kses191130' || $code=='smc2019'){?>font-size:19px !important;<?}?>"><?=$setting_col['question_icon_txt']?></a>
		<?}?>
		</li>
