...
while(is_array($col = mysqli_fetch_array($result))) {
	...
	$temp_result = mysqli_query($conn, "select count(*) cnt from session_favor_tbl WHERE code='".$code."' and session_sid='".$col['sid']."' and deviceid='".$deviceid."'");
	...
	if($cnt3==1 && ($tab=="-2" || $tab=="-3" || $tab=="-5")){
		if( ($tab=="-3" || $tab=="-2") && $result->num_rows ){
	?>
...
<div class="session" id="session<?=$col['sid']?>">
	...
	<?
		...
		$totalnum = $subresult->num_rows;

	?>	
	
	<a <?if($totalnum>0){?> href="#" class="trigger"<?}?>><?if($totalnum>0){?> <i class="fas fa-angle-down"></i><?}?>
	...
	<?}?>
		...
		<?if($setting_col['program_view_type']=="1"){
			...
			$faculty_view_type=1;
			//include "./speaker.php";
			include $_SERVER['DOCUMENT_ROOT']."/php/session/speaker.php";
	...
	<?if($tab != "-7"){?>
		...
		<?if($col['room_view']=="Y"){?>
			...
			if($code=="ksc2019"){
				...
				if(in_array($col['sid'], $ex_room1)) { $room_info = "Abstract/Case Zone 1 (".$room_info.")"; }
				else if(in_array($col['sid'], $ex_room2)) { $room_info = "Abstract/Case Zone 2 (".$room_info.")"; }
				else if(in_array($col['sid'], $ex_room3)) { $room_info = "Abstract/Case Zone 3 (".$room_info.")"; }
				else if(in_array($col['sid'], $ex_room4)) { $room_info = "Abstract/Case Zone 4 (".$room_info.")"; }
				else if(in_array($col['sid'], $ex_room_a)) { $room_info = "Moderated Poster Zone A (".$room_info.")"; }
				else if(in_array($col['sid'], $ex_room_b)) { $room_info = "Moderated Poster Zone B (".$room_info.")"; }
				else if(in_array($col['sid'], $ex_room_c)) { $room_info = "Moderated Poster Zone C (".$room_info.")"; }
				else if(in_array($col['sid'], $ex_room_d)) { $room_info = "Moderated Poster Zone D (".$room_info.")"; }
				

					
			?>
				
				<li class="view<?=$col['view_type']?>"><?if($col['room_photo']){?><a href="/upload/room/<?=$col['room_photo']?>"><?}?><i class="fas fa-map-marker-alt" title="Venue"></i> <?=$room_info?><?if($col['room_photo']){?></a><?}?></li>
		...
		<?if($col['liveYN']=='Y') {
			...
			if(strpos($_SERVER['HTTP_USER_AGENT'],'iPod') || strpos($_SERVER['HTTP_USER_AGENT'],'iPhone')) {
				$live_url = $col['live_url_ios'];

			} else {
				$live_url = $col['live_url_and'];
			}
		?>
			<?if($live_url) {?>
			<li style="margin-top:10px;float:right;display:inline;padding:0;">
				<a href="live.php?url=<?=urlencode($live_url)?>">
				<img src="/image/icon_live.png" alt="" style="height:26px;vertical-align:top;">
				</a>
			</li>
			
			<!-- <li style="margin-top:10px; padding-left:0;clear: both;"><a class="" style="margin-top:15px" href="live.php?url=<?=urlencode($live_url)?>"><img src="/image/icon_live.png" alt="" style="height:28px;vertical-align:top;"></a></li> -->
			<?}?>
		<?}?>

		<?if($setting_col['session_evaluation']=='Y' && $col['sc_viewYN']=='Y'){?>
		<li style="margin-top:10px; padding-left:0;clear: both;"><a class="btnEvaluation" style="margin-top:-15px" onclick="javascript:session_evaluation('<?=$col['sid']?>','<?=$deviceid?>','<?=$code?>')"><i class="fas fa-star"></i><?=$setting_col['session_evaluation_txt']?></a></li>
	
		<?}?>

		<?if($setting_col['session_question']=="1" && $col['qna_viewYN']=='Y'){ 
		if($col['sid']=='7478' || $col['sid']=='7582') {$q_link = "sid=".$col['sid'].'&voting=Y';}
		else $q_link = "sid=".$col['sid'];
		?>
		<li style="margin-top:10px;float:right;"><a href="app_question.php?<?=$q_link?>" class="btnQuestion" style="margin-top:-15px;<?if($code=='kses190818' || $code=='kses191130' || $code=='smc2019'){?>font-size:19px !important;<?}?>"><?=$setting_col['question_icon_txt']?></a></li>
	...
	<div id="session<?=$col['sid']?>_sub" class="toggleCon" <?if($glance){?>style="display:block"<?}?>>
		...
		<?}?>
			...
			<?if($setting_col['sub_theme_type']=="2" && $col['memo']){?>
		...
		?>
			...
			<li><a href="javascript:move_page('/php/session/view.php?sid=<?=$subcol['sid']?>&toptext=<?=$toptext?>&code=<?=$code?>&deviceid=<?=$deviceid?>&tab=<?=$tab?>&glanceYN=<?=$glanceYN?>','session<?=$col['sid']?>')"><i class="fas fa-angle-right"></i>
