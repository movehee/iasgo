...
if($subcol['abs_no']){
$abs_query = "select * from abstract_tbl WHERE abs_no = '".$subcol['abs_no']."'";
...
<div class="session">
	...
	<ul class="sessionUtil">
		...
		<?if($subcol['lecture_file']){?>
			<li><a href="/upload/session/<?=$subcol['lecture_file']?>" class="btnLect" target="_blabk;"><i class="fas fa-book"></i> <?=$setting_col['lecture_txt']?> <?if($subcol['lecture_file2']){?> 1<?}?></a></li>
		<?}?>
		<?if($subcol['lecture_file2']){?>
			<li><a href="/upload/session/<?=$subcol['lecture_file']?>" class="btnLect"><i class="fas fa-book"></i> <?=$setting_col['lecture_txt']?> 2</a></li>
		<?}?>

		<?if($subcol['abstract_file']){?>
			<li><a href="/upload/session/<?=$subcol['abstract_file']?>" class="btnAbs"><i class="far fa-file-alt"></i> <?=$setting_col['abstract_txt']?></a></li>
		<?}?>

		<?if(file_exists("../../upload/code/".$code."/".str_replace("]","",str_replace("[","",$subcol['abs_no'])).".pdf")){?>
			<li><a href="/upload/code/<?=$code?>/<?=str_replace("]","",str_replace("[","",$subcol['abs_no']))?>.pdf" class="btnAbs"><i class="far fa-file-alt"></i> <?=$setting_col['abstract_txt']?></a></li>
		<?}?>
	...
	<?if(($setting_col['abs_info1'] && $abs_col['info1']) || ($setting_col['abs_info2'] && $abs_col['info2']) || ($setting_col['abs_info3'] && $abs_col['info3']) || ($setting_col['abs_info4'] && $abs_col['info4']) || ($setting_col['abs_info5'] && $abs_col['info5']) || ($setting_col['abs_info6'] && $abs_col['info6']) || ($setting_col['abs_info7'] && $abs_col['info7']) || ($setting_col['abs_info8'] && $abs_col['info8']) || ($setting_col['abs_info9'] && $abs_col['info9']) || ($setting_col['abs_info10'] && $abs_col['info10']) ){?>
	<div class="abstractCon plus<?=$_COOKIE['abs_font']?>" id="abstractCon">
		...
		<dl class="<?if($setting_col['abs_align']=="1"){?>txtAlignJ<?}else{?>txtAlignL<?}?>">
