...
<?
	...
	if($_SERVER['REMOTE_ADDR']=='218.235.94.220'){
		//$query .= " and chking!='Y' ";
		$query .= " and score is null  and t1.day='2' and t2.member_level!='M'";
	}
	...
	<input type="hidden" name="result_code" value="<?=$result_code?>">
		<fieldset>
			...
			<div class="btn btnArea">
				...
				<input type="button" value="Excel Backup" onclick="location.href='excel_backup.php'" class="btnMint initialism fade_open btn btn-success ex_btn">
			</div>
		</fieldset>
	</form>
</div>
...
<table class="tblDef" >
	...
	<tbody>
		...
		<tr <?if($d['modify']=='Y'){?>style="background:#EDEDED"<?}?>>
			...
			<td><?if($d['first_date']>0){?><?=date("H:i",$d['first_date'])?><?}?><!-- <br><?=$d['first_date']?> --></td>
			...
			<td <?if($i%2==0){?>style="background:#FEFFE1;"<?}?>>
				<?if($d['s'.$i.'_sdate']>0){?>
					<?if($d['day']=='2' && $i=='3'){?>
						<?
							if($d['s'.$i.'_sdate']>strtotime("2020-11-29 14:30:00")){
								echo "";
							}else{
								echo date("H:i",$d['s'.$i.'_sdate']);
							}
						?>
					<?}else{?>
						<a href="javascript:popup_call('registration/session_time','sid=<?=$d['sid']?>&day=<?=$d['day']?>')"><?=date("H:i",$d['s'.$i.'_sdate'])?></a>
					<?}?>
				<?}?>
			</td>
			...
			<td <?if($i%2==0){?>style="background:#FEFFE1;"<?}?>>
				<?if($d['s'.$i.'_edate']>0){?>
					<?if($d['day']=='2' && $i=='3'){?>
						<?
							if($d['s'.$i.'_sdate']<strtotime("2020-11-29 14:50:00")){
								if($d['s'.$i.'_edate']>strtotime("2020-11-29 14:50:00")){
									echo "14:50";
								}else{
									echo date("H:i",$d['s'.$i.'_edate']);
								}
							}
						?>
					<?}else{?>
					<a href="javascript:popup_call('registration/session_time','sid=<?=$d['sid']?>&day=<?=$d['day']?>')"><?=date("H:i",$d['s'.$i.'_edate'])?></a>
					<?}?>
				<?}?>
