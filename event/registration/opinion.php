	...
	<input type="hidden" name="result_code" value="<?=$result_code?>">
		<fieldset>
				...
				<tbody>
					...
					<tr>
						...
						<td class="al" colspan="7">
							<select name="reg_kind" style="height:30px;">
								<option value="">선택</option>
								<?foreach($_REG['reg_kind'] as $tkey=>$tval){?>
								<option value="<?=$tkey?>" <?if($reg_kind==$tkey){?>selected<?}?>><?=$tval?></option>
								<?}?>
							</select>
						</td>
					</tr>
			...
			<div class="btn btnArea">
				...
				<input type="button" value="Excel Backup" onclick="location.href='excel_opinion.php?<?=$search_url?>'" class="btnGreen2 initialism fade_open btn btn-success ex_btn">
