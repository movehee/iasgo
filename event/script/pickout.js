...
var pickout = (function(){
	...
	function createOption(data, modal, config){
		...
		_.events(item, 'click', function(e){
			...
			if (config.multiple) {
				...
				//var kind = _.attr(select, 'id');
				//var kind_val = select[data.index].value;
				
				//pickout_controller(kind,kind_val);
			}
	...
	function setOptionSimple(select, data, txt){
		...
		pickout_controller(kind,kind_val); //김영환 추가함.
	...
	function feedField(select, value){
		//alert(_.attr(select.parentElement.querySelector('.pk-field'),'id'));
