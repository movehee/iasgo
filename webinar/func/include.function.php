...
function admin_orderby($naming,$field,$sort_field,$orderby,$search_url){
	if($field==$sort_field) $color="style='color:red;'";
	if($orderby=="" || $orderby=="desc" || $field!=$sort_field){
		echo "<a href='".$PHP_SELF."?sort_field=".$field."&orderby=asc".$search_url."' ".$color.">".$naming."↓</a>";
	}else{
		echo "<a href='".$PHP_SELF."?sort_field=".$field."&orderby=desc".$search_url."' ".$color.">".$naming."↑</a>";
	}
}
...
function imgResize_cdn($img,$limit_w,$limit_h,$opar=100){
	$size = getImagesize($img);
	...
	//return '<a href="'.$file_img.'" target="_blank"><img src="'.$file_img.'" width="'.$x_size.'" height="'.$y_size.'"></a>';
	if($x_size>0 && $y_size>0){
		return '<img src="'.$img.'" width="'.$x_size.'" height="'.$y_size.'" style="border:0px;opacity:'.$opar.'%;">';
	}
}
...
function Days_convert($time,$convert, $mType=null){

	$_Date['Month_eng'] = array("01"=>"January","02"=>"Febuary","03"=>"March","04"=>"April","05"=>"May","06"=>"June","07"=>"July","08"=>"August","09"=>"September","10"=>"October","11"=>"November","12"=>"December");
	$_Date['Month_short_eng'] = array("01"=>"Jan","02"=>"Feb","03"=>"Mar","04"=>"Apr","05"=>"May","06"=>"June","07"=>"July","08"=>"Aug","09"=>"Sep","10"=>"Oct","11"=>"Nov","12"=>"Dec");
	$_Date['days_eng'] = array("1"=>"Mon","2"=>"Tue","3"=>"Wed","4"=>"Thu","5"=>"Fri","6"=>"Sat","0"=>"Sun");
	if(strpos($time,'-')>0){
	...
	$week = date('w', $strtime);
	//echo date('Y-m-d',$time)." ".$_Date['Month_short_eng'][date('m',$time)].' '.date('j',$time).' ('.$_Date['days_eng'][$week].')';
	if($convert){
		if($mType == "s") {
			$time = str_replace("M",$_Date['Month_short_eng'][date('m',$strtime)],$convert);
		} else {
			$time = str_replace("M",$_Date['Month_eng'][date('m',$strtime)],$convert);
		}
				
		$time = str_replace("D",date('d',$strtime),$time);
		$time = str_replace("Y",date('Y',$strtime),$time);
		$time = str_replace("w",$_Date['days_eng'][$week],$time);
		echo $time;
	}else{
		echo date('Y-m-d',$strtime)." ".' ('.$_Date['days_eng'][$week].')';
	}
}

function eliminate_basic_tag($text){
	$txt = str_replace("<!DOCTYPE html>","",$text);
	$txt = preg_replace("!<DOCTYPE(.*?)>!is","",$txt);
	$txt = preg_replace("!<html(.*?)>!is","",$txt);
	$txt = preg_replace("!</html(.*?)>!is","",$txt);
	$txt = preg_replace("!<body(.*?)>!is","",$txt);
	$txt = preg_replace("!</body(.*?)>!is","",$txt);
	$txt = preg_replace("!<ul(.*?)>!is","",$txt);
	$txt = preg_replace("!</ul(.*?)>!is","",$txt);
	$txt = preg_replace("!<head(.*?)>!is","",$txt);
	$txt = preg_replace("!</head(.*?)>!is","",$txt);
	$txt = preg_replace("!<p(.*?)>!is","",$txt);
	$txt = preg_replace("!</p(.*?)>!is","",$txt);
