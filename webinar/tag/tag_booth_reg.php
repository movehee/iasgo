<?
	...
	if(!$tag_number){
		PutMessageLocation("정상적인 접근이 아닙니다.",'booth.php');
		exit;
	}
	...
	if(stristr($_SERVER['REMOTE_ADDR'],'218.235.94')==false){
		$attent = $conn->getOne("select event_chk from registration_tbl where sid='".$tag_number."'");
		if($attent!='Y'){
...
?>
<form name="tagF" id="tagF" method="post" action="tag_booth_result.php">
